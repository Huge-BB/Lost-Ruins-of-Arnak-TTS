...
const costKeys = new Set(['coin', 'compass', 'tablet', 'arrowhead', 'jewel']);

function allNodes(track) {
...
for (const boardId of ['bird', 'snake']) {
  ...
  for (const bridge of overlay.bridges ?? []) {
    ...
    seenBridges.add(id);
    if (!bridgeIds.has(id)) throw new Error(`${boardId}: manual bridge not found in generated topology: ${id}`);
  ...
  for (const override of overlay.nodeOverrides ?? []) {
    ...
    if (override.researchLevel !== undefined && (!Number.isInteger(override.researchLevel) || override.researchLevel < 0)) {
      throw new Error(`${boardId}:${override.node}: invalid researchLevel`);
    }
    if (override.spansLevels !== undefined) {
      if (!Array.isArray(override.spansLevels) || override.spansLevels.length === 0) {
        throw new Error(`${boardId}:${override.node}: spansLevels must be a non-empty array`);
      }
      for (const level of override.spansLevels) {
        if (!Number.isInteger(level) || level < 0) throw new Error(`${boardId}:${override.node}: invalid spansLevels entry`);
      }
    }
  }
