...
import { setupLeader, runLeaderRoundStart, runLeaderRoundEnd } from './index.ts';
import {
...
test('Captain setup has three archaeologists and specialist is once per round', () => {
  ...
  assert.equal((state.players.p1 as any).leader.data.specialistUsedThisRound, false);
});
...
test('Falconer eagle advances at round start, caps, and returns for a chosen reached reward', () => {
  ...
  assert.equal((state.players.p1 as any).leader.data.eaglePosition, 1);
  state = falconerAdvanceEagle(state, 'p1', 10);
  assert.equal((state.players.p1 as any).leader.data.eaglePosition, 4);
  state = falconerReturnEagle(state, 'p1', 3);
  assert.equal((state.players.p1 as any).leader.data.eaglePosition, 0);
  assert.deepEqual(state.pendingRewards[0].payload, { rewardPosition: 3, mainAction: true });
});

test('Baroness starts with Special Delivery in hand and gains round II-V income', () => {
  const state = baseState();
  setupLeader(state, 'p1', 'baroness', context, 'seed');
  assert.equal(context.cards[state.players.p1.hand[0]].name, 'Special Delivery');
  state.round = 2;
  runLeaderRoundStart(state, 'p1', context);
...
test('Professor creates a three-card archive and receives suitcase bonuses', () => {
  ...
  assert.deepEqual((state.players.p1 as any).leader.data.archive, ['a1', 'a2', 'a3']);
  state.round = 2;
  runLeaderRoundStart(state, 'p1', context);
  assert.equal((state.players.p1 as any).leader.data.suitcase.compass, 1);
...
test('Mystic starts each round with an extra Fear and supports all three ritual tiers', () => {
  let state = baseState();
  setupLeader(state, 'p1', 'mystic', context, 'seed');
  runLeaderRoundStart(state, 'p1', context);
  assert.equal(state.players.p1.hand.filter(id => context.cards[id]?.type === 'Fear').length, 1);
  (state.players.p1 as any).leader.data.ritualPile = ['fear1', 'fear1', 'fear1', 'fear1'];
  state = mysticPerformRitual(state, 'p1', 2);
  assert.equal(state.players.p1.resources.coin, 1);
  assert.equal(state.players.p1.resources.compass, 1);
  (state.players.p1 as any).leader.data.ritualPile = ['fear1', 'fear1', 'fear1'];
