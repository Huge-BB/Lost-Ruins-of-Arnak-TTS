...
import { useLeaderIdol, type IdolEffect } from './idol-actions.ts';
...
export function reduceExpeditionLeaderAction(state:GameState,action:ExpeditionLeaderAction,context:EngineContext):GameState{
 ...
 switch(action.type){
  ...
  case'LEADER_USE_IDOL':return useLeaderIdol(state,action.playerId,action.idolId,action.slotIndex,action.effect,context,{snackId:action.snackId,ritualFearCount:action.ritualFearCount});
