...
export function resolvePendingFreeGuardian(state:GameState,playerId:PlayerId,pendingIndex:number,siteId:string):GameState{
 ...
 const isResearch=payload?.type==='OVERCOME_GUARDIAN_FREE';
 const isMystic=pending.code==='leader:MYSTIC_OVERCOME_GUARDIAN_FREE';
 if(!isResearch&&!isMystic)throw new Error('Pending reward is not a free guardian action');
 const site=state.sites[siteId];
 if(!site)throw new Error(`Unknown site: ${siteId}`);
 if(site.occupiedBy!==playerId)throw new Error('Free guardian action requires your archaeologist at the site');
 if(!site.guardian)throw new Error('There is no guardian at the site');
 const next=structuredClone(state); const guardianId=next.sites[siteId].guardian!;
 delete next.sites[siteId].guardian;
 next.players[playerId].defeatedGuardians.push(guardianId);
