...
export function applyResearchManualData(track:ResearchTrackDefinition,manualData:ResearchManualData,options:{requireVerified?:boolean}={}):ResearchTrackDefinition{
 ...
 for(const manualBridge of overlay.bridges){ const key=bridgeKey(manualBridge.from,manualBridge.to); if(seenBridges.has(key)) throw new Error(`Duplicate manual research bridge: ${key}`); seenBridges.add(key); let target=topology.get(key); if(!target&&manualBridge.to===researchTempleNode(boardId)&&nodes.has(manualBridge.from)){target={id:key,from:manualBridge.from,to:manualBridge.to};bridges.push(target);topology.set(key,target);} if(!target) throw new Error(`Manual research bridge does not exist in ${boardId} topology: ${key}`); validateCost(manualBridge.cost,key); if(requireVerified&&!manualBridge.verified) throw new Error(`Research bridge is not verified: ${key}`); target.cost={...manualBridge.cost}; target.rewards=manualBridge.rewards ? structuredClone(manualBridge.rewards) : undefined; target.verified=manualBridge.verified; }
 for(const manualReward of overlay.nodeRewards??[]){ const target=nodes.get(manualReward.node); if(!target) throw new Error(`Manual research reward node does not exist in ${boardId} topology: ${manualReward.node}`); const key=`${manualReward.node}:${manualReward.token??'any'}`; if(seenRewards.has(key)) throw new Error(`Duplicate manual research node reward: ${key}`); seenRewards.add(key); if(requireVerified&&!manualReward.verified) throw new Error(`Research node reward is not verified: ${key}`); target.rewards??=[]; target.rewards.push({token:manualReward.token,rewards:structuredClone(manualReward.rewards),verified:manualReward.verified}); }
 return next;
}
